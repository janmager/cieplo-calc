...
function DetailedRaportAdmin({data, print = false, printAuto = false}:{data: Raport, print?: any, printAuto?: boolean}) {
    ...
    return (
        <div className='flex flex-col gap-5 mt-[-20px]'>
            ...
            <CustomAdminDetailsInputData label='ID Raportu' data={data.human_id} />
            ...
            {data.recommendedProducts && <div className='flex flex-col gap-7'>
                <CustomAdminDetailsDivider text='Sugerowane urządzenia do Twojego budynku' />
                <div className="flex flex-col gap-5 mt-[-20px]">
                    {
                        (data.recommendedProducts && data.recommendedProducts != '[]') ? JSON.parse(data.recommendedProducts).map((item: any, index: number) => {
                            return (
            ...
            { data.api_heated_area && <CustomAdminDetailsInputData checkBox={true} label='Ogrzewana powierzchnia budynku' data={data.api_heated_area ? `${data.api_heated_area} m²` : '-'} />}
            ...
            { data.api_total_area && <CustomAdminDetailsInputData checkBox={true} label='Całkowita powierzchnia budynku' data={data.api_total_area ? `${data.api_total_area} m²` : '-'} />}
            ...
            <CustomAdminDetailsInputData checkBox={true} label='Całkowita (razem z ew. dociepleniem) grubość ścian zewnętrznych' col={true} data={data.total_wall_thickness ? `${data.total_wall_thickness} cm` : ''} />
            ...
            {(!data.heat_demand_know && data.heating_levels.indexOf('Poddasze') == -1 && data.house_roof_plan && data.house_roof_plan.toLowerCase() == 'skośny z poddaszem') && <CustomAdminDetailsInputData checkBox={true} col={true} label='Sytuacja na nieogrzewanym poddaszu' data={data.unheated_space_type} />}
            ...
            {data.building_type && <>
                ...
                <CustomAdminDetailsInputData col={true} checkBox={true} label='Rodzaj instalacji centralnego ogrzewania' data={data.type_of_heating_instalation} />
                ...
                <CustomAdminDetailsInputData checkBox={true} label='Oblicz zapotrzebowanie energii na CWU' data={data.count_need_energy_cwu ? 'yes' : 'no'} />
            </>}
            {

    ...
    <div className='flex flex-col items-center w-full'>
        <div style={{backgroundImage: `url(${bg.src})`, backgroundAttachment: 'fixed', backgroundSize: 'cover',}} className='w-full relative flex items-end justify-center h-[625px]'>
            <div className='max-w-[1172px] px-5 xl:px-0 text-white flex flex-col gap-8 pb-8 z-20'>
                <h1 className='text-[80px] font-bold leading-[110%]'>Kalkulator doboru<br/>pompy ciepła</h1>
        ...
        <div className='py-10 px-5 w-full max-w-[1172px] flex flex-col lg:grid grid-cols-1 lg:grid-cols-2'>
            ...
            <div className='flex w-full flex-col gap-3 lg:col-span-2 mt-7'>
                ...
                <div className={`max-h-[480px] transition-all cursor-pointer duration-300 bg-gray-100 ${!formData['house_location'] ? 'grayscale hover:grayscale-0' : ``}`}>
                    <div id="map" style={{width: "100%", height: "450px"}}/>
                </div>
            </div>
            <div className='flex w-full flex-col gap-3 lg:col-span-2 mt-10'>
                <CustomLabel label={'Podaj zapotrzebowanie cieplne swojego budynku:'} />
                <div className='grid grid-cols-1 md:grid-cols-2 mt-5 gap-5 md:gap-0'>
                    <div className='flex flex-col pl-0'>
                        <div className='md:max-w-[275px] flex flex-row items-start gap-3 cursor-pointer opacity-90 hover:opacity-100 duration-200 transition-all' onClick={() => setFormData({...formData, heat_demand: {...formData.heat_demand, know: true}})}>
                    ...
                    <div className='flex flex-col border-0 md:border-l-2 pl-0 md:pl-10'>
                        <div className='md:max-w-[320px] flex flex-row items-start gap-3 cursor-pointer opacity-90 hover:opacity-100 duration-200 transition-all' onClick={() => setFormData({...formData, heat_demand: {...formData.heat_demand, know: false}})}>
                            <div className='relative min-h-[18px] min-w-[18px] max-h-[18px] max-w-[18px]'>
                                <Image src={checkSelectBorder.src} className={formData.heat_demand && !formData.heat_demand.know ? '' : 'opacity-50'} height={18} width={18} alt={'border'} />
                                {formData.heat_demand && !formData.heat_demand.know && <Image className='absolute left-[3.5px] top-[4.5px]' src={checkSelectDot.src} height={10} width={10} alt={'dot'} />}
                            </div>
            ...
            <div className='col-span-2 flex justify-end items-end mt-5'>
                <NextButton active={formData['house_location'] && formData.heat_demand} setViewId={setViewId} nextView={2} />
            </div>
        </div>
    </div>
  )
